...
void init() {
	...
	if(rw !is null)
		ringworldType = rw.id;
	// [[ MODIFY BASE GAME START ]]
	// do the same for dyson spheres, except the planet type definition uses
	// Photovoltics
	auto@ dysonSphere = getPlanetType("Photovoltics");
	if (dysonSphere !is null) {
		dysonSphereType = dysonSphere.id;
	}
	// [[ MODIFY BASE GAME END ]]
}
...
// [[ MODIFY BASE GAME START ]]
// Add dyson sphere version, using the same ringworld materials
class DrawDysonSphere : Draw3D {
	DrawDysonSphere(Planet@ pl) {
	}
...
Draw3D@ makeDrawMode(Object@ obj) {
	...
	switch(obj.type) {
		case OT_Planet: {
			...
			else if (pl.PlanetType == dysonSphereType) {
				return DrawDysonSphere(pl);
			}
			else {
