...
class StatusFile : FileDef {
	...
	StatusFile() {
		...
		block("Status", doc="A type of status effect that can be added to an object.",
			...
			field("Show Duration", AT_Boolean, "False", doc="The duration of instances of this status should be visible to end users. (WIP: Only implemented for status boxes on ships and planets)");
...
class ArtifactFile : FileDef {
	ArtifactFile() {
		...
		block("Artifact", doc="A type of artifact that can be found and used.").checkDuplicates(artifactCompletions);
			...
			// [[ MODIFY BASE GAME START ]]
			// include new field for creating deep space artifacts
			field("Ignored By Gravitars", AT_Boolean, "False", doc="If this artifact is a deep space artifact that should be ignored by Gravitars and spawned separately to stellar artifacts.");
			// [[ MODIFY BASE GAME END ]]
