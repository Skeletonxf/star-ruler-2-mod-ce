...
void loadCompletions(array<Completion@>@ arr, const string& folder, const string& blockName) {
	...
	for(uint i = 0, cnt = list.length; i < cnt; ++i) {
		...
		while(file++) {
			...
			// [[ MODIFY BASE GAME START ]]
			else if(file.key.equals_nocase("Point Cost")) {
				compl.pointCost = toDouble(file.value);
			}
			else if(file.key.equals_nocase("Time Cost")) {
				compl.timeCost = toDouble(file.value);
			}
			// [[ MODIFY BASE GAME END ]]
		}
	}
