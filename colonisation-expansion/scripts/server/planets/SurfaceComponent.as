...
tidy class SurfaceComponent : Component_SurfaceComponent, Savable {
	...
	// [[ MODIFY BASE GAME START ]]
	void swapBiome(Object& obj, uint old_id, uint new_id) {
		uint index_change = 99;
		if (biome0 == old_id) {
			index_change = 0;
		} else if (biome1 == old_id) {
			index_change = 1;
		} else if (biome2 == old_id) {
			index_change = 2;
		} else {
			return;
		}
		...
		auto@ new_biome = getBiome(new_id);
		if (new_biome is null) {
			return;
		}
		...
		for (uint tile = 0, cnt = grid.biomes.length; tile < cnt; ++tile) {
			if (grid.biomes[tile] == old_id) {
				grid.biomes[tile] = new_id;
			}
		}
		grid.delta = true;

		// Update the biome id references and modification id ??
		++SurfaceModId;
		if(biomes.length >= 1)
			biome0 = biomes[0].id;
		if(biomes.length >= 2)
	...
	void surfaceTick(Object& obj, double time) {
		...
		// bombard effects should not be ignored just because a planet has
