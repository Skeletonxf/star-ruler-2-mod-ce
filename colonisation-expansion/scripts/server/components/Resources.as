...
tidy class ObjectResources : Component_Resources, Savable {
	...
	// Allow reversing ChangeNativeResourceTo effects by
	// making the terraforming fully reversible, by caching
	// all the removed resources here, but this will be lost
	// if further terraforming is attempted before the
	// ChangeNativeResourceTo effect is disabled.
	///
	// Examples
	// Terraform before removing Transmuter:
	// Grain + Water --Transmuter-> Base Materials --Terraform-> Oil -> --Remove Transmuter-> Oil
	...
	void load(SaveFile& file) {
		...
		// [[ MODIFY BASE GAME START ]]
		uint resourcesRemembered = 0;
		file >> resourcesRemembered;
	...
	void terraformTo(Object& obj, uint resId) {
		...
		// [[ MODIFY BASE GAME ]]
		// Lose any previously remembered resources
		rememberedResources.resize(0);
		for(uint i = 0, cnt = nativeResources.length; i < cnt; ++i) {
			if(!nativeResources[i].type.artificial) {
				obj.removeResource(nativeResources[i].id);
				// Remember all resources we remove
				rememberedResources.insertLast(nativeResources[i].type.id);
			}
		}
		// [[ MODIFY BASE END ]]
	...
	// [[ MODIFY BASE GAME START ]]
	void terraformBack(Object& obj, uint resId) {
		// Try to remove the resource we're supposed to reverse
		// the terraforming of
		bool stillHasResource = false;
		for(uint i = 0, cnt = nativeResources.length; i < cnt && !stillHasResource; ++i) {
			if(nativeResources[i].type.id == resId) {
		...
		if (!stillHasResource) {
			// resource no longer present to terraform back
			terraforming = false;
			return;
		}

		// Add back the remembered resources
		for(uint i = 0, cnt = rememberedResources.length; i < cnt; ++i) {
			obj.addResource(rememberedResources[i]);
		}
