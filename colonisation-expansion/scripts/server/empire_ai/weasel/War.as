	...
	bool tick(AI& ai, War& war, double time) {
		...
		if(defendPlanet is null) {
			...
			for(uint i = 0, cnt = system.planets.length; i < cnt; ++i) {
				...
				w *= capt;
				// [[ MODIFY BASE GAME START ]]
				// Planets being bombarded are effectively 'lost' a lot
				// faster than planets being sieged
				if (pl.bombardment > 0) {
					w *= 1.5;
				}
				// [[ MODIFY BASE GAME END ]]
...
class War : AIComponent, WarAI {
	...
	void focusTick(double time) override {
		...
		// [[ MODIFY BASE GAME START ]]
		// Check for planets getting bombarded a little bit all the time
		// and regularly when fighting battles
		for (uint i = 0, cnt = battles.length + 4; i < cnt; ++i) {
			checkPlanetsUnderBombardment();
		}
		// [[ MODIFY BASE GAME END ]]
	}

	// [[ MODIFY BASE GAME START ]]
	void checkPlanetsUnderBombardment() {
		if (bombardPlanetCheck < planets.planets.length) {
			PlanetAI@ plAI = planets.planets[bombardPlanetCheck];
			bombardPlanetCheck += 1;
			if (plAI is null) {
				return;
			}
