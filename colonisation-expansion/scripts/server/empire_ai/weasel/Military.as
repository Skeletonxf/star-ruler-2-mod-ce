...
class Military : AIComponent, IMilitary { // [[ MODIFY BASE GAME START ]]
	Fleets@ fleets;
...
interface IMilitary {
	/**
	 * Gets the staging base in/for this region, if any
	 */
	StagingBase@ getBase(Region@ inRegion);

	/**
	 * Returns where in a region to station a thing.
	 */
	vec3d getStationPosition(Region& inRegion, double distance = 100.0);

	/**
	 * Stations a fleet into a region
	 */
	void stationFleet(FleetAI@ fleet, Region@ inRegion);
	...
	/**
	 * Gets or makes a staging base for this region
	 */
	Region@ getStagingFor(Region& targetRegion);

	/**
	 * Gets the closest staging base for this region, if one is in range.
	 */
	Region@ getClosestStaging(Region& targetRegion);
}
// [[ MODIFY BASE GAME END ]]
