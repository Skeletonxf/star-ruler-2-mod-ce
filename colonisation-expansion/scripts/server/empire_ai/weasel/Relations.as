...
final class Relation {
	...
	void tick(AI& ai, Relations& relations, double time) {
		...
		if (ai.behavior.hostile) {
			// hostile AIs declare war on sight
			if (!atWar && canDeclareWar(ai)) {
				relations.declareWar(empire, aggressive=true);
			}
		}
	...
	void focusTick(double time) override {
		...
		if(gameTime > treatyRespond) {
			...
			if(respondTreaty !is null) {
				...
				if(respondTreaty.hasClause("SubjugateClause") && !ai.behavior.hostile) { // [[ MODIFY BASE GAME ]]
				...
				else if(respondTreaty.hasClause("PeaceClause") && !ai.behavior.hostile) { // [[ MODIFY BASE GAME ]]
		...
		if(gameTime > treatyConsider) {
			...
			for(uint i = 0, cnt = relations.length; i < cnt; ++i) {
				...
				if(other.atWar && !ai.behavior.hostile) { // [[ MODIFY BASE GAME ]]
				...
				// No point demanding surrender for campaign, so hostile
				// also prevents a hostile AI subjugating another empire
				if(other.atWar && !ai.behavior.hostile) { // [[ MODIFY BASE GAME ]]
