...
final class FleetIntel {
	...
	bool hasCarpetBombs(AI& ai) {
		if (obj !is null && obj.valid && obj.isVisibleTo(ai.empire)) {
			if (obj.isShip) {
...
final class Intel {
	...
	// [[ MODIFY BASE GAME START ]]
	// Perhaps this should have an expiry but it's probably better to
	// go on the safe side and assume anyone making carpet bombs plans on
	// using them properly - the AI doesn't use carpet bombs yet so
	// this is effectively a 'has a player tried to use a mechanic' flag
	// right now.
	bool hasMadeCarpetBombs = false;
	// TODO: This would be a good place for saving and gathering intel
	// on other things like Senatorial Palaces
	// [[ MODIFY BASE GAME END ]]
	...
	void focusTick(AI& ai, Intelligence& intelligence) {
		...
		if (!hasMadeCarpetBombs) {
			for(uint i = 0, cnt = fleets.length; i < cnt; ++i) {
				if (fleets[i].hasCarpetBombs(ai)) {
					hasMadeCarpetBombs = true;
					break;
				}
			}
		}
