...
class MapGeneration {
	...
	// [[ MODIFY BASE GAME START ]]
	// adapted from game_start.as
	SystemDesc@ getClosestSystem(const vec3d& point) {
		SystemDesc@ closest;
		double dist = INFINITY;
		for(uint i = 0, cnt = systems.length; i < cnt; ++i) {
			double d = systems[i].position.distanceToSQ(point);
			if(d < dist) {
				dist = d;
				@closest = systems[i];
			}
	...
	void generateRougeObjects() {
		...
		for (uint i = 0; i < rougeObjects; ++i) {
			vec3d pos = vec3d(randomd(leftExtent.x, rightExtent.x), 0, randomd(leftExtent.z, rightExtent.z));
			auto@ closest = getClosestSystem(pos);
			pos.y = closest.position.y;
