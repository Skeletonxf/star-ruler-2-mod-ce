...
tidy class RegionObjects : Component_RegionObjects, Savable {
	...
	// Do the same for orbitals, also, what is this undeclared type
	// magic???
	OrbitalBucket orbitalBucket;
...
#section server
	void load(SaveFile& msg) {
		...
		// [[ MODIFY BASE GAME START ]]
		// Do the same for orbitals
		msg >> cnt;
		orbitalList.length = cnt;
		for(uint i = 0; i < cnt; ++i) {
			msg >> orbitalList[i];
			if(orbitalList[i] !is null)
				orbitalBucket.add(orbitalList[i]);
		}
		// [[ MODIFY BASE GAME END ]]
...
#section all
	...
	// [[ MODIFY BASE GAME START ]]
	// Do the same for orbitals
	uint get_orbitalCount() const {
		return orbitalBucket.length;
	}

	Orbital@ get_orbitals(uint index) const {
		if(index >= orbitalBucket.length)
			return null;
		return orbitalBucket[index];
	}
	// [[ MODIFY BASE GAME END ]]
...
#section all
	...
	void getOrbitals() {
		for(uint i = 0, cnt = orbitalBucket.length; i < cnt; ++i)
			yield(orbitalBucket[i]);
	}
