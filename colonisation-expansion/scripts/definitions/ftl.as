...
int hyperdriveCost(Object& obj, const vec3d& position) {
	...
	double reductionFlagCostFactor = 1.0;
...
double hyperdriveRange(Object& obj, int scale, int stored) {
	...
	return sqr(max(double(stored) - (HYPERDRIVE_START_COST - owner.HyperdriveStartCostMod * reductionFlagCostFactor) * owner.FTLCostFactor, 0.0) / (log(double(scale)) * HYPERDRIVE_COST * owner.FTLCostFactor));
	// [[ MODIFY BASE GAME END ]]
}
...
int flingCost(Object& obj, vec3d position) {
	...
	if(obj.isShip) {
		...
		if(obj.isOrbital)
			return ceil(FLING_COST * obj.radius * 3.0 * owner.FTLCostFactor * reductionFlagCostFactor); // [[ MODIFY BASE GAME ]]
		else if(obj.isPlanet)
			return ceil(FLING_COST * obj.radius * 30.0 * owner.FTLCostFactor * reductionFlagCostFactor); // [[ MODIFY BASE GAME ]]
		return INFINITY;
	}
}
...
int slipstreamCost(Object& obj, int scale, double distance) {
	...
	if(distance < optDist)
		return baseCost * obj.owner.FTLCostFactor * reductionFlagCostFactor; // [[ MODIFY BASE GAME ]]
	return baseCost * ceil(distance / optDist) * obj.owner.FTLCostFactor * reductionFlagCostFactor; // [[ MODIFY BASE GAME ]]
}
...
double slipstreamRange(Object& obj, int scale, int stored) {
	...
	if(stored < baseCost)
		return 0.0;
	return floor(double(stored) / baseCost / obj.owner.FTLCostFactor * reductionFlagCostFactor) * optDist; // [[ MODIFY BASE GAME ]]
}
