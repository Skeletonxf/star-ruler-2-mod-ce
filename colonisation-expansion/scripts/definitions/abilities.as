...
void loadAbilities(const string& filename) {
	...
	while(file++) {
		...
		// [[ MODIFY BASE GAME START ]]
		// Parse the additional fields added to AbilityType
		else if(key.equals_nocase("Resource Transfer")) {
			type.isResourceTransfer = toBool(value);
		}
		else if(key.equals_nocase("Resource")) {
			@type.resource = getResource(value);
