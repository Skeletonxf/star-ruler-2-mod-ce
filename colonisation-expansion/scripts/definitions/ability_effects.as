...
#section server
	...
	void tick(Ability@ abl, any@ data, double time) const {
		...
		if(target.hasOrbit) {
			...
			// [[ MODIFY BASE GAME START ]]
			// target.impulse actually applies an acceleration to the target (it's
			// not an impulse since the units it takes are `m*s^-1` instead of
			// `kg*m*s^-1`).
			// maxAcceleration is already calculated as `thrust / (mass + bonus mass)`
			// so combined, these two factors mean the vector argument we give to
			// impulse will scaled down to the acceleration we're currently able to move
			// at, in units of `m*s^-1`.
			// [[ MODIFY BASE GAME END ]]
			if(abl.obj.hasMover)
