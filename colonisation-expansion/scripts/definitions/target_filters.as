...
// [[ MODIFY BASE GAME START ]]
class TargetFilterNoCargoStorage : TargetFilter {
	Document doc("Only allow targets that do not have cargo storage.");
	Argument objTarg(TT_Object);

	string getFailReason(Empire@ emp, uint index, const Target@ targ) const override {
		return locale::NTRG_NO_CARGO;
	}
...
class TargetFilterNotHasCargoStored : TargetFilter {
	Document doc("Only allow targets that do not have some cargo stored.");
	Argument objTarg(TT_Object);
