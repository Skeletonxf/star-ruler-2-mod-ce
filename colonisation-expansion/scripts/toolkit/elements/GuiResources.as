...
// Add icon display logic for (Sun)Light resource class
const ResourceClass@ lightClass;
...
void init() {
	...
	// [[ MODIFY BASE GAME START ]]
	@lightClass = getResourceClass("Light");
	// [[ MODIFY BASE GAME END ]]
...
void drawSmallResource(const ResourceType@ type, const Resource@ r, const recti& pos, Object@ drawFrom = null, bool onPlanet = false) {
	if(r !is null) {
		...
		if(drawFrom !is null && !r.usable && r.origin is null) {
			...
			// [[ MODIFY BASE GAME START ]]
			else if(r.type.cls is lightClass) {
				LIGHT_REQ.draw(pos);
			}
			// [[ MODIFY BASE GAME END ]]
...
const Sprite LIGHT_REQ(material::LightResourceClass);
...
Sprite getAutoImportIcon(const AutoImportDesc& desc) {
	if(desc.type !is null) {
		...
		if(desc.cls is foodClass)
			...
			return WATER_REQ;
		else if(desc.cls is lightClass)
			return LIGHT_REQ;
		return Sprite();
	}
