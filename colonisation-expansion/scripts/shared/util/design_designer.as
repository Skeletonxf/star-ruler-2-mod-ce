...
// This doesn't seem to be used for designing anything?
...
class Designer {
	...
	void prepare(DesignType Type, double size, Empire@ emp, const string& ClassName, const string& Name = "") {
		...
		// [[ MODIFY BASE GAME START ]]
		favorJumpdriveOverHyperdrive = emp.hasTrait(getTraitID("Extragalatic"));
		// [[ MODIFY BASE GAME END ]]
	...
	bool _designFlagship() {
		...
		if(hyperTypes.length != 0) {
			...
			// [[ MODIFY BASE GAME START ]]
			bool hyperTypesIncludesHyperdrive = false;
			bool hyperTypesIncludesJumpdrive = false;

			for (uint i = 0; i < hyperTypes.length-1; i++) {
				auto@ hyperType = hyperTypes[i];
				if (hyperType.name == "Hyperdrive") {
					hyperTypesIncludesHyperdrive = true;
				}
				if (hyperType.name == "Jumpdrive") {
					hyperTypesIncludesJumpdrive = true;
				}
			}
			...
			// [[ MODIFY BASE GAME END ]]
			auto@ hyperType = hyperTypes[index];
